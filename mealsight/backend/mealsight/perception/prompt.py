...
def build_extraction_prompt(transcript: str) -> str:
    ...
    return (
        ...
        "field, empty) whenever it wasn't mentioned at all. This applies just as "
        "much to mood_or_preference as any other field: wanting something easy, "
        "quick, or made from whatever's on hand is NOT evidence of tiredness or "
        "any other emotional state — never write \"tired\" (or infer any mood at "
        "all) unless the person's own words actually name one.\n"
        "\n"
        "Every stated constraint must end up SOMEWHERE in the output — never "
        "silently dropped. If something is clearly stated but doesn't map "
        "cleanly onto any specific field below (dietary_restrictions, "
        "avoid_ingredients, avoid_dishes, and the rest), put it in "
        "additional_context instead of leaving it out entirely. A constraint "
        "landing in additional_context is always better than it vanishing from "
        ...
        "- avoid_ingredients: list of specific ingredients OR ingredient-like "
        "categories/qualities actually stated as something to avoid — not just "
        'single named items: "no seafood" belongs here as "seafood", "not too '
        'spicy" belongs here as "excess spice", the same way "no peanuts" '
        'belongs here as "peanuts". This only covers something traceable to a '
        "specific ingredient, additive, or seasoning dimension (spice/heat "
        "level, sweetness, saltiness, a named allergen or category of food) — "
        "a general statement about how the MEAL AS A WHOLE should feel "
        '("not too heavy", "nothing too filling", "something light") is a '
        "mood/preference, not an ingredient to avoid, and belongs in "
        "mood_or_preference instead, even though both are phrased as a "
        '"not too X" complaint. Only promote something to dietary_'
        "restrictions instead when it names a recognized whole-category diet "
        '(vegetarian, keto, and similar) — "no seafood" is what NOT to '
        "include, not a stated diet, so it stays here even though it is "
        "broader than one ingredient\n"
